...
import { bibleStore } from '../world-building/stores/bible-store'; // reaches into another module's internals
```
...
// ❌ FORBIDDEN — world-building module pulling workspace state directly
// src/modules/world-building/components/CharacterCard.svelte
import { activeSceneId } from '$modules/workspace/stores/workspace';
```
...
- Database queries (SQLite/Dexie) beyond the `+page.ts` load function (move to service layer)
...
A `services/` file inside a module handles **all domain operations for that module** — API calls, DB interactions, transformations, validations — but has no Svelte-specific code (`$state`, `$derived`, event dispatching). Services are plain TypeScript.

```ts
// ✅ CORRECT — pure TS service
// src/modules/world-building/services/character-service.ts
import { apiClient } from '$lib/api-client';

export async function getCharactersByProject(projectId: string) {
        return apiClient.get(`/api/db/characters?projectId=${projectId}`);
}

export async function createCharacter(data: NewCharacter) {
        const id = crypto.randomUUID();
        await apiClient.post('/api/db/characters', { ...data, id, createdAt: Date.now() });
        return id;
}
```
...
| ------------------------------------------ | ---------------------------------- |
| Which character is selected in World Building | `src/modules/world-building/stores/` |
...
| Database queries outside `+page.ts`, service files, or `+server.ts`               | Data access must be traceable and mockable                           |
