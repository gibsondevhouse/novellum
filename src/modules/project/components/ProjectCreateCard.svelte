...
<div class="overlay" role="dialog" aria-modal="true" aria-labelledby="create-dialog-title">
	<!-- Backdrop dismiss -->
	<div class="backdrop" role="presentation" onclick={handleCancel}></div>

	<div class="card">
		<!-- Header -->
		<div class="card-header">
			<h2 class="card-title" id="create-dialog-title">New Project</h2>
			<GhostButton type="button" class="btn-close" onclick={handleCancel} aria-label="Close">✕</GhostButton>
		</div>
		...
		<form class="card-body" onsubmit={handleSubmit} novalidate>
			...
			<section class="form-section" aria-label="Core identity">
				<SectionHeader title="Your Story" class="create-section-header" />
			...
			{#if showStory}
				<section class="form-section story-section" aria-label="Story intent">
					<SectionHeader title="Story" class="create-section-header" />
...
<style>
	...
	:global(.btn-close:focus-visible) {
	...
	:global(.create-section-header) {
		border-bottom: none;
		padding-bottom: 0;
		margin-bottom: var(--space-4);
	}

	:global(.create-section-header .title) {
	...
	:global(.advanced-toggle:focus-visible) {
		outline: 2px solid var(--color-border-focus);
		outline-offset: 2px;
