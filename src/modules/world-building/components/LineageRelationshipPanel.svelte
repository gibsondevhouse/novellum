<script lang="ts">
	...
	function saveNewRelationship() {
		if (!newTargetLineageId) {
			formError = 'Select a target lineage first.';
			return;
		}
		...
		onAddRelationship({
			targetLineageId: newTargetLineageId,
...
<section class="dossier-section" aria-label="Relationships">
	<SectionHeader title="Relationships" class="dossier-section-header">
		{#snippet actions()}
			<GhostButton
				...
				aria-controls="lineage-relationships-content"
	...
	{#if !isCollapsed}
		<div id="lineage-relationships-content">
			...
			{:else}
				...
				{:else}
					<div class="create-form">
						<CharacterSelect
							id="new-lineage-target"
							label="Target lineage"
							value={newTargetLineageId}
							options={lineages}
							excludeId={currentLineageId}
							onChange={(id) => (newTargetLineageId = id)}
						/>
						<input
							class="input-inline"
							type="text"
							list="lineage-relationship-types"
							value={newRelationshipType}
							oninput={(e) => (newRelationshipType = (e.currentTarget as HTMLInputElement).value)}
							placeholder="Relationship type"
						/>
						<input
							class="input-inline"
							type="text"
							list="lineage-relationship-status"
							value={newStatus}
							oninput={(e) => (newStatus = (e.currentTarget as HTMLInputElement).value)}
							placeholder="Status"
						/>
						<textarea
							class="input-inline"
							rows="2"
							value={newNotes}
							oninput={(e) => (newNotes = (e.currentTarget as HTMLTextAreaElement).value)}
							placeholder="Notes"
						></textarea>
						{#if formError}
			...
			<datalist id="lineage-relationship-types">
				{#each relationshipTypeSuggestions as s (s)}
					<option value={s}></option>
				{/each}
			</datalist>
			<datalist id="lineage-relationship-status">
				{#each statusSuggestions as s (s)}
					<option value={s}></option>
				{/each}
			</datalist>
					...
					{#each lineage.relationships as rel, index (rel.id)}
						<div class="relationship-item">
							<div class="rel-header">
								<CharacterSelect
									id={`lineage-target-${rel.id}`}
									label="Target lineage"
									value={rel.targetLineageId || ''}
									options={lineages}
									excludeId={currentLineageId}
									onChange={(id) => onRelationshipFieldChange(index, 'targetLineageId', id)}
								/>
								<input
									class="input-inline"
									type="text"
								...
								<input
									...
									value={rel.status || ''}
									oninput={(e) =>
										onRelationshipFieldChange(index, 'status', (e.currentTarget as HTMLInputElement).value)
									}
									placeholder="Status"
								/>
								<GhostButton
							...
							<textarea
								...
								value={rel.notes || ''}
								oninput={(e) =>
									onRelationshipFieldChange(index, 'notes', (e.currentTarget as HTMLTextAreaElement).value)
								}
