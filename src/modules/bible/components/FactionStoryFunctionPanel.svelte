...
<section class="dossier-section" aria-labelledby="faction-story-title">
	<div class="section-header"><h3 id="faction-story-title" class="section-title">Story Function</h3><button type="button" class="collapse-toggle" aria-expanded={!isCollapsed} aria-controls="faction-story-content" onclick={() => (isCollapsed = !isCollapsed)}>{isCollapsed ? 'Expand' : 'Collapse'}</button></div>
	{#if !isCollapsed}
