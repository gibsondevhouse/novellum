...
<section class="dossier-section" aria-labelledby="faction-state-title">
	<div class="section-header"><h3 id="faction-state-title" class="section-title">Current State</h3><button type="button" class="collapse-toggle" aria-expanded={!isCollapsed} aria-controls="faction-state-content" onclick={() => (isCollapsed = !isCollapsed)}>{isCollapsed ? 'Expand' : 'Collapse'}</button></div>
	{#if !isCollapsed}
