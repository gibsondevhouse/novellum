...
<section class="dossier-section" aria-labelledby="lineage-story-title">
	<div class="section-header"><h3 id="lineage-story-title" class="section-title">Story Function</h3><button type="button" class="collapse-toggle" aria-expanded={!isCollapsed} aria-controls="lineage-story-content" onclick={() => (isCollapsed = !isCollapsed)}>{isCollapsed ? 'Expand' : 'Collapse'}</button></div>
	{#if !isCollapsed}
