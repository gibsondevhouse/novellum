...
<section class="dossier-section" aria-labelledby="faction-relationships-title">
	<div class="section-header"><h3 id="faction-relationships-title" class="section-title">Relationships</h3><button type="button" class="collapse-toggle" aria-expanded={!isCollapsed} aria-controls="faction-relationships-content" onclick={() => (isCollapsed = !isCollapsed)}>{isCollapsed ? 'Expand' : 'Collapse'}</button></div>
	{#if !isCollapsed}
					...
					{#each faction.relationships as rel, index (rel.id)}
						<div class="relationship-item">
							<div class="rel-header">
								...
								<button type="button" class="secondary-button" aria-label={`Remove relationship with ${resolveFactionName(rel.targetFactionId)}`} onclick={() => onRemoveRelationship(index)}>Remove</button>
							</div>
...
<style>
	...
	.create-button,.secondary-button { border: 1px solid color-mix(in srgb, var(--color-border-default) 75%, transparent); background: transparent; padding: 0.25rem 0.6rem; font-size: var(--text-xs); border-radius: var(--radius-sm); cursor: pointer; color: var(--color-text-primary); }
