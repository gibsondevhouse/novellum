...
<section class="dossier-section" aria-labelledby="faction-members-title">
	<div class="section-header"><h3 id="faction-members-title" class="section-title">Members / Notable Figures</h3><button type="button" class="collapse-toggle" aria-expanded={!isCollapsed} aria-controls="faction-members-content" onclick={() => (isCollapsed = !isCollapsed)}>{isCollapsed ? 'Expand' : 'Collapse'}</button></div>
	{#if !isCollapsed}
