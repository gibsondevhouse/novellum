...
<section class="dossier-section" aria-labelledby="lineage-members-title">
	<div class="section-header"><h3 id="lineage-members-title" class="section-title">Members / Descendants / Notable Figures</h3><button type="button" class="collapse-toggle" aria-expanded={!isCollapsed} aria-controls="lineage-members-content" onclick={() => (isCollapsed = !isCollapsed)}>{isCollapsed ? 'Expand' : 'Collapse'}</button></div>
	{#if !isCollapsed}
