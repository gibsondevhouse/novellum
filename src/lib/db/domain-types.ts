/**
 * Canonical Novellum domain types.
 *
 * These interfaces describe the project-owned entity shapes used across both
 * the SQLite server layer (`$lib/server/db/*`) and the V1 client modules. They
 * are intentionally framework-agnostic — no Dexie, no SvelteKit, no Vitest.
 *
 * All `createdAt` / `updatedAt` fields are ISO 8601 strings, matching the
 * existing SQLite TEXT columns. List-shaped fields are typed here as native
 * arrays; repositories are responsible for JSON (de)serialization at the
 * database boundary.
 */
