<script lang="ts">
	...
	function startWorldbuildWithNova(): void {
		...
		novaPanel.openWithPrompt(
			'Help me build a World Bible for this project. Guide me through each domain in order: Personae (characters, factions, lineages), Atlas (locations, realms, landmarks), The Archive (myths, technology, traditions), Threads (plot arcs, motivations, sub-plots), and Chronicles (eras, key events, personal histories). Start by reviewing my existing worldbuilding context and identifying which domain would benefit most from development.',
		);
	}
	...
	<div class="landing-page">
		<section class="hero" aria-labelledby="worldbuilding-hero-title">
			...
			<div class="hero-actions" role="navigation" aria-label="Jump to world-building domains">
				{#each sections as section (section.id)}
					...
					<a
						...
						aria-label={pendingCount > 0
							? `${section.label}, ${formatPendingSuggestionLabel(pendingCount, section.label)}`
							: section.label}
					>
						{section.label}
				...
				{/each}
				<button type="button" class="hero-jump hero-jump--nova" onclick={startWorldbuildWithNova}>
