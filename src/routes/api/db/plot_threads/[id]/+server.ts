...
export const GET: RequestHandler = async ({ params }) => {
	const row = db.prepare('SELECT * FROM plot_threads WHERE id = ?').get(params.id) as
...
export const PUT: RequestHandler = async ({ params, request }) => {
	...
	const simpleAllowed = ['title', 'description', 'status'];

	for (const key of simpleAllowed) {
		if (key in body) updates[key] = body[key];
	}

	const jsonFields = ['relatedSceneIds', 'relatedCharacterIds'];
	...
	const stmt = db.prepare(`UPDATE plot_threads SET ${setClauses.join(', ')} WHERE id = @id`);
	...
	const updated = db.prepare('SELECT * FROM plot_threads WHERE id = ?').get(params.id) as Record<
		string,
		unknown
	>;
...
export const DELETE: RequestHandler = async ({ params }) => {
	db.prepare('DELETE FROM plot_threads WHERE id = ?').run(params.id);
