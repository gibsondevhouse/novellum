...
export const GET: RequestHandler = async ({ params }) => {
	const row = db.prepare('SELECT * FROM consistency_issues WHERE id = ?').get(params.id) as
...
export const PUT: RequestHandler = async ({ params, request }) => {
	...
	const simpleAllowed = ['type', 'severity', 'description', 'sceneId', 'status'];

	for (const key of simpleAllowed) {
		if (key in body) updates[key] = body[key];
	}

	const jsonFields = ['entityIds'];
	...
	const stmt = db.prepare(`UPDATE consistency_issues SET ${setClauses.join(', ')} WHERE id = @id`);
	...
	const updated = db
		.prepare('SELECT * FROM consistency_issues WHERE id = ?')
		.get(params.id) as Record<string, unknown>;
	return json(decodeRow(updated));
};

export const DELETE: RequestHandler = async ({ params }) => {
	db.prepare('DELETE FROM consistency_issues WHERE id = ?').run(params.id);
