...
export const GET: RequestHandler = async ({ params }) => {
	const row = db.prepare('SELECT * FROM timeline_events WHERE id = ?').get(params.id) as
...
export const PUT: RequestHandler = async ({ params, request }) => {
	...
	const simpleAllowed = ['title', 'description', 'date'];

	for (const key of simpleAllowed) {
		if (key in body) updates[key] = body[key];
	}

	const jsonFields = ['relatedCharacterIds', 'relatedSceneIds'];
	...
	const stmt = db.prepare(`UPDATE timeline_events SET ${setClauses.join(', ')} WHERE id = @id`);
	...
	const updated = db.prepare('SELECT * FROM timeline_events WHERE id = ?').get(params.id) as Record<
		string,
		unknown
	>;
